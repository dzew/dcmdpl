...
/**
 * A Factory for CosineVFs.
 * 
 * @author Dawit
 * 
 */
public class CosineVfFactory implements LvfFactory {
     ...
     */
    public CosineVfFactory(Cell domain, int terms) {
        this.domain = domain;
    ...
    @Override
    public CosineVF construct(SimpleMatrix coeffVector) {
        double[] coeffs = new double[coeffVector.getNumElements()];
        for (int i = 0; i < coeffs.length; i++) {
            coeffs[i] = coeffVector.get(i);
        }
        return CosineVF.of(Vector.asVector(coeffs), domain, harmonics);
    }
